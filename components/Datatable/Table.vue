            ...
            <tr>
                <th v-for="(col, ind) in columns" :key="'col-' + ind">
                    <template v-if="col.type === 'checkbox'">
                        <input type="checkbox" ref="checkedAll" :class="theme.tableCheckBox"
                            @click="checkedList = (checkedAll.checked === true) ? flatByProp(col.prop) : []">
                    </template>
                    <template v-else>
                        <div :class="theme.tableSort">
                            <span>{{ col.text }}</span>
                            <span v-if="sort.col === col.prop && sort.by === 'asc'"
                                @click="sort.col = col.prop; sort.by = 'desc'; emit('sort', sort);">
                            ...
                            <span v-else-if="sort.col === col.prop && sort.by === 'desc'"
                                @click="sort.col = col.prop; sort.by = 'asc'; emit('sort', sort);">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    ...
                    <th v-for="(col, ind) in columns" :key="'filter-' + ind">
                        <div v-if="col.filter === true && 'prop' in col">
                            <div :class="theme.tableSelect" v-if="col.filterType === 'select'">
                                <select :class="theme.tableSelectBox" v-model="filterMap[col.prop]"
                                    @change="emit('filter', filterMap)">
                            ...
                            <template v-else>
                                <input type="text" v-model="filterMap[col.prop]" :class="theme.tableInputBox"
                                    @input="emit('filter', filterMap)">
                            </template>
                        </div>
                    </th>
                </tr>
            ...
            <tr v-for="(entry, index) in entries" :key="'entry-' + index">
                <td v-for="(col, ind) in columns" :key="'col-' + ind">
                    ...
                    <template v-else-if="col.type === 'checkbox'">
                        <input type="checkbox" :class="theme.tableCheckBox" :ref="setCheckedRef" :value="entry[col.prop]"
                            :checked="checkedList.includes(entry[col.prop])"
                            @click="(checkedRefs[ind].checked === true) ? checkedList.push(entry[col.prop]) : removeChecked(entry[col.prop]); emit('checklist', checkedList)">
                    </template>
